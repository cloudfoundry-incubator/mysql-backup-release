...
import (
	...
	"streaming-mysql-backup-client/name"
...
func DefaultClient(config config.Config) *Client {
	return NewClient(
		...
		name.NewArtifactNamer(config),
	)
}

func NewClient(config config.Config, tarClient *tarpit.TarClient, backupPreparer BackupPreparer, downloader Downloader, galeraAgentCaller GaleraAgentCallerInterface, namer name.ArtifactNamer) *Client {
	client := &Client{
		config:            config,
		tarClient:         tarClient,
		backupPreparer:    backupPreparer,
...
/*
func (c Client) artifactName( name string) string {
...
func (c Client) encryptedBackupLocation(name string) string {
	return path.Join(c.config.OutputDir, fmt.Sprintf("%s.tar.gpg", name))
}
...
func (c Client) finalMetadataLocation(name string) string {
	return path.Join(c.config.OutputDir, fmt.Sprintf("%s.txt", name))
}
...
func (c *Client) BackupNode(ip string, index int) error {
	var err error
	name, err := c.namer.Name(ip, index)
	if err != nil {
		return err
	}
	...
	err = c.writeMetadataFile(ip, name)
	if err != nil {
		return err
	}

	// tarAndEncryptBackup(ip, name)
	err = c.tarAndEncryptBackup(ip, name)
	if err != nil {
		return err
	}
	return nil
...
func (c *Client) writeMetadataFile(ip string, name string) error {
	src := c.originalMetadataLocation()
	dst := c.finalMetadataLocation(name)
...
func (c *Client) tarAndEncryptBackup(ip string, name string) error {
	c.logInfo(ip, "Starting encrypting backup")

	tarCmd := c.tarClient.Tar(c.prepareDirectory)

	encryptedFileWriter, err := os.Create(c.encryptedBackupLocation(name))
	if err != nil {
